...
#pragma warning restore 0219
        ...
        Expression PromoteExpression(Expression expr, Type type, bool exact)
        {
            ...
            if (expression != null)
            {
                ...
                {
                    string name;
                    if (literals.TryGetValue(ce, out name))
                    {
                        ...
                        switch (Type.GetTypeCode(ce.Type))
                        {
                            ...
                            case TypeCode.Int64:
                            case TypeCode.UInt64:
                                value = ParseNumber(name, target);
                                break;
                            case TypeCode.Double:
                                if (target == typeof(decimal)) value = ParseNumber(name, target);
                                break;
                            case TypeCode.String:
                                value = ParseEnum(name, target);
                                break;
                        }
                        if (value != null)
            ...
            if (!IsCompatibleWith(expr.Type, type)) return null;
